...
def make_files():
    ...
    for i in range(data["count"]):
        ...
        if checkout("cd {}; dd if=/dev/urandom of={} bs={} count=1 iflag=fullblock".format(data["FOLDER_TST"], filename,\
                                                                                           data["size"]), ""):
...
def make_subfolder():
    ...
    if not checkout("cd {}; mkdir {}".format(data["FOLDER_TST"], subfoldername), ""):
        return None, None
    if not checkout("cd {}/{}; dd if=/dev/urandom of={} bs={} count=1 iflag=fullblock".\
                            format(data["FOLDER_TST"], subfoldername, testfilename, data["size"]), ""):
...
def make_bad_arx(make_folders, clear_folders,make_files):
    checkout("cd {}; 7z a {}/badarx.7z".format(data["FOLDER_TST"], data["BAD_7z"]), "Everything is Ok")
    yield checkout("truncate -s 1 {}/badarx.7z".format(data["BAD_7z"]), ""), "test1 FAIL"
